        ...
        std::vector<std::string> &negativeExamplesPaths)
{
    ...
    if(is_directory(inputPath))
    {
        ...
        {
            printf("start");
//            inputPathFilename.push_back((char)'\0');
            printf("%s",inputPathFilename.c_str());
            printf(inriaPersonPositivesPath.filename().c_str());
            printf("end");
        }
#endif
...
void bootstrapTrain(const int verbose, const bool doBootstrap = true)
{
    ...
    /// calculate maxNumExample = pos + stages * num_negative_image_per_bootstrap_stage
    ...
    /// yl get the different features ( different square windows in an image )
    ...
    /// training data is cutted from the Offset and scaled by Factor = 4 always in this program
