...
void IntegralChannelsForPedestrians::set_image(const boost::gil::rgb8c_view_t &the_input_view)
{
    ...
    channels.resize(boost::extents[num_channels][channel_size.y][channel_size.x]); //yl shrinked by factor=4 usually
    integral_channels.resize(boost::extents[num_channels][channel_size.y+1][channel_size.x+1]);//yl shrinked as above,but plus 1 for margin
    input_channels.resize(boost::extents[10][input_size.y][input_size.x]);// yl no shrinking now
...
void IntegralChannelsForPedestrians::compute_v1()
{
    ...
    compute_hog_channels_v1();// no shrink
    compute_luv_channels_v0();// no shrink
    resize_channels_v1();// shrinking
    integrate_channels_v0();// have been shrinked, calc the integral
