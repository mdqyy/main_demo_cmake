...
void ObjectsDetectionApplication::main_loop()
{
    ...
    stixels_t stixels_from_previous_frame;// vector<Stixels>
    ground_plane_corridor_t ground_corridor_from_previous_frame; // vector<int>
...
void ObjectsDetectionApplication::one_step()
{
    ...
    AddBorderFunctor add_border(additional_border);
    static bool video_input_is_available = false;
    if(should_process_folder)
    {
        video_input_is_available = directory_input_p->next_frame();
    }
    ...
    printf("Average objects detection compute only speed per iteration %.2lf [s] (in the %i 'th iterations)\n",
           cumulated_objects_detector_compute_time , num_iterations );
...
cv::Mat ObjectsDetectionApplication::get_image()
{
    AbstractVideoInput::input_image_view_t input_view;
...
const doppia::AbstractObjectsDetector::detections_t& ObjectsDetectionApplication::get_detections()
{
